...
workstream x skillset x market rate x hours, and reconciles to the $400,000
par-value basis (10,000,000 x $0.04).
...
SHARES = 10_000_000
PAR = 0.04  # -> $400,000 stated capital
...
def build() -> Workbook:
    ...
    ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=6)
    ws.cell(row=row, column=1, value="PAR-VALUE RECONCILIATION").font = _BOLD
    ws.cell(row=row, column=1).fill = _SUB_FILL
    row += 1
    recon = [
        ("Shares issued", SHARES, "#,##0"),
        ("Par value per share", PAR, '"$"0.0000'),
        ("Stated capital (shares × par)", None, _MONEY),  # formula below
        ("Schedule grand total", None, _MONEY),
        ("Variance (total − stated capital)", None, _MONEY),
    ]
    r0 = row
    ws.cell(row=r0, column=1, value=recon[0][0]).font = _BOLD
    ws.cell(row=r0, column=6, value=SHARES).number_format = "#,##0"
    ws.cell(row=r0 + 1, column=1, value=recon[1][0]).font = _BOLD
    ws.cell(row=r0 + 1, column=6, value=PAR).number_format = '"$"0.0000'
    ws.cell(row=r0 + 2, column=1, value=recon[2][0]).font = _BOLD
    ws.cell(row=r0 + 2, column=6, value=f"=F{r0}*F{r0 + 1}").number_format = _MONEY
    ws.cell(row=r0 + 3, column=1, value=recon[3][0]).font = _BOLD
    ws.cell(row=r0 + 3, column=6, value=f"=F{grand}").number_format = _MONEY
    ws.cell(row=r0 + 4, column=1, value=recon[4][0]).font = _BOLD
    ws.cell(row=r0 + 4, column=6, value=f"=F{grand}-F{r0 + 2}").number_format = _MONEY
    row = r0 + 6

    for note in [
        "Par value ($0.04 × 10,000,000 = $400,000) is the stated-capital basis for the share",
        "issuance. This schedule provides defensible, hours-based support for that value.",
